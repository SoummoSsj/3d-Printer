...
class Detector:
	...
	def detect(self, frame_bgr: np.ndarray, conf: float = 0.25) -> List[Tuple[int,int,int,int,float,int]]:
		res = self.model.predict(source=frame_bgr, imgsz=640, conf=conf, verbose=False)[0]
